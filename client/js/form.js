...
(function () {
  ...
  const fields = ['penyakit', 'nama_kabupaten_kota', 'jumlah_kasus', 'tahun', 'pelapor'];
  ...
  function validate(d) {
    ...
    if (!d.tahun || Number(d.tahun) < 2018 || Number(d.tahun) > CURRENT_YEAR) e.tahun = `Tahun 2018–${CURRENT_YEAR}`;
    return e;
  }
  ...
  async function loadList() {
    ...
    try {
      ...
      if (!rows.length) { tbody.innerHTML = `<tr><td colspan="6" class="empty-state">Belum ada laporan. Tambahkan lewat form di atas.</td></tr>`; return; }
    ...
    } catch (e) { tbody.innerHTML = `<tr><td colspan="6" class="empty-state">Gagal memuat. Pastikan server berjalan.</td></tr>`; }
  }
  ...
  async function startEdit(id) {
    try {
      ...
      form.querySelector('[name="tahun"]').value = r.tahun;
  ...
  form.addEventListener('submit', async (ev) => {
    ...
    const payload = { penyakit: data.penyakit, nama_kabupaten_kota: data.nama_kabupaten_kota, jumlah_kasus: Number(data.jumlah_kasus), tahun: Number(data.tahun), pelapor: data.pelapor || null };
    try {
  ...
  (async function init() {
    ...
    if (tahun) { const now = new Date().getFullYear(); tahun.innerHTML = '<option value="" disabled selected>Pilih tahun</option>'; for (let y = now; y >= 2018; y--) tahun.insertAdjacentHTML('beforeend', `<option value="${y}">${y}</option>`); }
    loadList();
  })();
})();
